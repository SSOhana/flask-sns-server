...
class PostingResource(Resource) :
    ...
    def post(self) :
        ...
        # 3. 오브젝트 디텍션을 수행해서 레이블의 Name 을 가져온다. (인공지능 활용, 예측)
        ...
        response = client.detect_labels(Image={'S3Object' : {
                                        ...
                                        'Name':new_file_name }} , 
                                        MaxLabels=5 )   # 하나에 이미지에 태그 5개 뽑는 것

        # 4. 레이블의 Name 을 가지고 태그를 만든다 !!! (엄청 중요한 부분!)

        # 4-1. label['Name'] 의 문자열을 tag_name 테이블에서 찾는다. (반복문을 통해서 처리 해야,)
        ...
        for label in response['Labels'] :
            # label['Name'] 이 값을 우리는 태그 이름으로 사용할 것
            try :
                connection = get_connection()
    ...
    def get(self) :
        ...
        try :
            ...
            # 2. 쿼리문 만들기
            query = ''' select p.*, u.email, u.name, count(l.postingId) as like cnt
                    from posting p
                    join user u
                    on p.userId = u.id
                    join likes l
